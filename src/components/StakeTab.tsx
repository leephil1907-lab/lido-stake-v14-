  ...
  return (
    <div className="max-w-xl mx-auto px-3 sm:px-4 py-6 sm:py-8 animate-in fade-in duration-300">
      <div className="text-center mb-6 sm:mb-8">
        ...
        <p className="text-sm sm:text-base text-text-secondary">Stake ETH and receive stETH while staking</p>
      </div>
      ...
      <div className="bg-card rounded-[24px] p-4 sm:p-6 mb-6 sm:mb-8 border border-border-main shadow-2xl relative overflow-hidden">
        ...
        {isConnected ? (
          <button 
            ...
            className={`w-full min-h-[52px] py-3.5 px-4 text-base sm:text-lg rounded-2xl mb-6 font-bold transition-all shadow-md flex items-center justify-center gap-2 select-none cursor-pointer active:scale-[0.98] ${(!ethAmount || Number(ethAmount) <= 0 || isPending) ? 'bg-[#00A3FF]/40 text-white/80 cursor-not-allowed' : 'bg-[#00A3FF] hover:bg-[#0090E6] text-white shadow-[#00A3FF]/25'}`}
          >
            {isPending && <RefreshCw className="w-5 h-5 animate-spin" />}
            <span>{isPending ? 'Confirming in Wallet...' : 'Stake ETH'}</span>
          </button>
        ) : (
