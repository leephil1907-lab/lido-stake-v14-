...
export function WithdrawalsTab() {
    ...
    try {
      ...
      if (address) {
        recordFailedTransaction({
          ...
          amount: amount ? `${amount} stETH` : undefined,
          token: 'stETH',
          errorMessage: err.shortMessage || err.message || 'Withdrawal request failed',
          severity: 'warning',
        });
      }
      if (err.message) {
    ...
    try {
      ...
      if (address) {
        recordFailedTransaction({
          ...
          amount: `${claimAmount} ETH`,
          token: 'ETH',
          errorMessage: err.shortMessage || err.message || 'Withdrawal claim failed',
          severity: 'warning',
        });
      }
      if (err.message) {
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-8 animate-in fade-in duration-300">
      ...
      <div className="bg-card rounded-[24px] p-2 sm:p-6 mb-8 border border-border-main shadow-sm">
